...
combined$Duration <- combined$duration_ms
...
write_rds(grouping3, "Data_Final_Project/grouping3.rds")
...
grouping1 %>%
  filter(decade != "1940") %>%
            ...
            ggplot(aes(fill = not_hot100, x = decade, y = mean_attribute)) +
            geom_col(position = "dodge")
...
model_1 <- lm(`Peak Position` ~ danceability * energy * speechiness * valence, data = grouping2)
